        ...
        <div className="w-full md:w-1/2 p-6 md:p-8 overflow-y-auto space-y-6">
          <div>
            <div className="flex items-center gap-2 text-xs text-orange-500 font-mono mb-2">
              ...
              <span>{product.category.toUpperCase()} • {product.gender.toUpperCase()}</span>
            </div>
            ...
            <div className="flex items-center gap-4 mt-3 text-xs text-slate-600 dark:text-white/60 border-b border-slate-200 dark:border-white/10 pb-4">
              <div className="flex items-center gap-1 text-orange-500">
                ...
                <span className="text-slate-400 dark:text-white/40">({product.reviewsCount} reviews)</span>
              </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              {product.colors.map((c, idx) => (
                ...
                >
                  <span className="w-3 h-3 rounded-full border border-slate-300 dark:border-white/20" style={{ backgroundColor: c.hex }} />
                  <span>{c.name}</span>
                </button>
              ))}
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-mono text-slate-500 dark:text-white/50 uppercase tracking-wider">
                SIZE: <span className="text-slate-900 dark:text-white font-bold">{selectedSize}</span>
              </label>
            </div>
            ...
            <div className="mt-3 text-xs text-slate-600 dark:text-white/60 leading-relaxed">
              ...
              {activeTab === 'fabric' && (
                <div className="space-y-1">
                  <p><strong className="text-slate-900 dark:text-white">Fabric:</strong> {product.fabric}</p>
                  <p><strong className="text-slate-900 dark:text-white">Care:</strong> {product.care}</p>
                  <p><strong className="text-slate-900 dark:text-white">Fit:</strong> {product.fit}</p>
                </div>
              )}
              {activeTab === 'shipping' && (
                <div className="space-y-1">
