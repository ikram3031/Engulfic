...
interface SystemUsersListResult {
  data: SystemUser[];
  meta?: {
    total: number;
    page: number;
...
const fetchSystemUsers = async (params?: FetchUsersParams): Promise<SystemUsersListResult> => {
  try {
...
export function useSystemUsers(params?: FetchUsersParams) {
  return useQuery<SystemUsersListResult>({
